...
class AccountLib:
    ...
    def get_account_for_credentials(self, login: str, password: str) -> AccountDocument:
        try:
            try_account: AccountDocument = self._kernel.server_db_session.query(AccountDocument).filter(
                or_(AccountDocument.email == login, AccountDocument.username == login),
            ).one()
        except NoResultFound:
            raise AccountNotFound()

        if not sha256(f"{password}{try_account.password_salt}".encode()).hexdigest() == try_account.password_hash:
            raise AccountNotFound()

        return try_account

    def username_exist(self, username: str) -> bool:
        return bool(self._kernel.server_db_session.query(AccountDocument).filter(AccountDocument.username == username).count())

    def email_exist(self, email: str) -> bool:
        return bool(self._kernel.server_db_session.query(AccountDocument).filter(AccountDocument.email == email).count())

    def create(self, username: str, email: str, password: str, commit: bool = True) -> AccountDocument:
    ...
    def get_account_for_id(self, account_id: str) -> AccountDocument:
        return self._kernel.server_db_session.query(AccountDocument).filter(
            AccountDocument.id == account_id
        ).one()
...
"""
        ...
        server = smtplib.SMTP(host=self._kernel.server_config.smtp_server, port=int(self._kernel.server_config.smtp_port))
        server.starttls()
