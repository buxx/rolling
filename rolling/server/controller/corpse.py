...
from rolling.model.corpse import GetAnimatedCorpsePath
from rolling.model.corpse import GetAnimatedCorpsesQuery
from rolling.model.corpse import SignalNewAnimatedCorpsePath
from rolling.model.event import NewAnimatedCorpseData
...
class AnimatedCorpseController(BaseController):
    ...
    @hapic.output_body(
        AnimatedCorpseModel, processor=RollingSerpycoProcessor(many=True)
    )
    async def get_animated_corpses(
        self, request: Request, hapic_data: HapicData
    ) -> typing.List[AnimatedCorpseDocument]:
        return self._kernel.animated_corpse_lib.get_all(
            world_row_i=hapic_data.query.world_row_i,
            world_col_i=hapic_data.query.world_col_i,
        )
    ...
    @hapic.handle_exception(NoResultFound, http_code=404)
    async def signal_new(self, request: Request, hapic_data: HapicData) -> Response:
        from rolling.server.application import HEADER_NAME__DISABLE_AUTH_TOKEN

        if (
            request.headers.get(HEADER_NAME__DISABLE_AUTH_TOKEN)
            != self._kernel.server_config.disable_auth_token
        ):
            return Response(status=403)

        animated_corpse = self._kernel.animated_corpse_lib.get(
            hapic_data.path.animated_corpse_id
        )
