...
from rolling.model.event import PlayerMoveData, ClientRequireAroundData, ThereIsAroundData
from rolling.model.event import ZoneEvent
from rolling.model.event import ZoneEventType
from rolling.model.serializer import ZoneEventSerializerFactory
from rolling.model.stuff import StuffModel
from rolling.server.controller.url import DESCRIBE_LOOT_AT_STUFF_URL, DESCRIBE_BUILD
...
class ThereIsAroundProcessor(EventProcessor):
    async def _process(
        self, row_i: int, col_i: int, event: ZoneEvent[ClientRequireAroundData], sender_socket: web.WebSocketResponse
    ) -> None:
        ...
        for stuff in stuffs:
            items.append((
                stuff.name,
                DESCRIBE_LOOT_AT_STUFF_URL.format(
                    character.id,
                    stuff.id,
                )
            ))
