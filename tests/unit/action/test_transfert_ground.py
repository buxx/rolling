...
def extract_button_url(description: Description, class_: str, identifier: str) -> str:
...
class TestTransfertGround:
            ...
            (
                ...
                [
                    ...
                    TransferOperation(
                        ...
                        after_ground_quantity=0.03,
                        after_carried_quantity=0.17,
                    ),
                    TransferOperation(
                        partial_take_count=1,
                        after_ground_quantity=0.02,
    ...
    async def test_transfer_partial_resource(
        ...
        # worldmapc_xena_haxe2: StuffModel,
        ...
        for transfer in transfers:
            ...
            if transfer.partial_deposit_count:
                ...
                deposit_url = extract_button_url(description, "partial_right", "WOOD")
                for _ in range(transfer.partial_deposit_count):
                    response = await web.post(deposit_url)
                    assert 200 == response.status

            if transfer.deposit_count:
                description = action.perform(xena,
                                             input_=TransfertGroundCharacterModel(parameters))
                deposit_url = extract_button_url(description, "right", "WOOD")
            ...
            if transfer.partial_take_count:
                description = action.perform(xena, input_=TransfertGroundCharacterModel(parameters))
                take_url = extract_button_url(description, "partial_left", "WOOD")
            ...
            if transfer.take_count:
                description = action.perform(xena,
                                             input_=TransfertGroundCharacterModel(parameters))
                take_url = extract_button_url(description, "left", "WOOD")
        ...
        # FIXME BS NOW: test resource line is deleted when 0.0
        #  and presence of all resource_id_partial_quantity
